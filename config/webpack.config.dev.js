...
module.exports = {
  ...
  module: {
    ...
    loaders: [
      // Default loader: load all assets that are not handled by other loaders with
      // the url loader.
      // Note: This list needs to be updated with every change of extensions the other
      // loaders match. E.g., when adding a loader for a new supported file extension,
      // we need to add the supported extension to this loader too. Add one new line
      // in `exclude` for each loader.
      //
      ...
      // would get copied to the `build` folder. "url" loader works like "file" loader
      // except that it embeds assets smaller than specified limit in bytes as data
      // URLs to avoid requests. A missing `test` is equivalent to a match.
      {
        exclude: [
      ...
      // JSON is not enabled by default in Webpack but both Node and Browserify allow
      // it implicitly so we also enable it.
      {
        test: /\.json$/,
        loader: 'json'
      },
  ...
  plugins: [
    // Makes the public URL available as %PUBLIC_URL% in index.html, e.g.: <link
    // rel="shortcut icon" href="%PUBLIC_URL%/favicon.ico"> In development, this
